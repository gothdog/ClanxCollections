...
/**
 * Index is the interface to a set of classes which provide implementations of multivalued maps
 * f(x) -> {y, z, ...} for use by MutableMultidex.
 *
 * @param <V>
 */
public interface Index<V> extends Queryable<V> {

  float getWeight();

  V getExactMatch(String key);

  V getNearestMatch(String key);

  RankedSet<V> getExactMatches(String key);

  RankedSet<V> getRankedMatches(String key);
}